...
\label{fig:tri}
\end{figure}
The above Fig. \ref{fig:tri} shows that, point D placed on side BC of $\triangle ABC$ such that AD = AC.\\
$\triangle ADC$ forms an isosceles triangle, where angles opposite to the equal sides are same.
\begin{align}
\angle ADC = \angle ACD
\label{eq:a1}
\end{align}
...
\implies \angle ADB = 180^o - \angle ABD - \angle BAD
\label{eq:a2}
\end{align}
At point D
\begin{align}
\angle ADB + \angle ADC = 180^o\\
\implies \angle ADC = 180^o - \angle ADB
\end{align}
By substituting \eqref{eq:a2}
\begin{align}
\angle ADC = 180^o - 180^o + \angle ABD + \angle BAD\\
\implies \angle ADC = \angle ABD + \angle BAD\\
\implies \angle ADC > \angle ABD
\end{align}
from \eqref{eq:a1}
\begin{align}
\angle ACD > \angle ABD
\label{eq:a3}
\end{align}
In a triangle, sides opposite to the highest angles are largest.\\
Hence, from \eqref{eq:a3}
\begin{align}
AB > AC\\
\therefore AB > AD
\end{align}
